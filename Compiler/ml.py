    ...
    def reveal_correctness(self, n=None, Y=None, debug=False):
        ...
        def _(i):
            ...
            if debug:
                ...
                print_ln_if(to_print, "%s: %s %s %s %s", i, truth, guess, b, nabla)
...
class MultiOutput(MultiOutputBase):
    ...
    def eval(self, N, top=False):
        ...
        if self.approx:
            ...
            @for_range_opt_multithread(self.n_threads, N)
            def _(i):
                relus = (self.X[i].get_vector() > 0).if_else(self.X[i].get_vector(), 0)
        ...
        @for_range_opt_multithread(self.n_threads, N)
        def _(i):
            x = self.X[i].get_vector() - util.max(
                self.X[i].get_vector()
            ).expand_to_vector(d_out)
            e = exp(x)
    ...
    def maybe_debug_backward(self, batch):
        if self.debug:
            ...
            def _(i):
                ...
                print_ln_if(check, "nabla X %s", self.nabla_X[i].reveal_nested())
...
class DenseBase(Layer):
    ...
    def reveal_parameters_to_binary(self, reshape=None):
        if reshape:
            ...
            O = trans.sizes[0]
            tmp = MultiArray(
                [O] + reshape, value_type=self.W.value_type, address=trans.address
            )
            X, Y, C = reshape
    ...
    def backward_params(self, f_schur_Y, batch):
        ...
        @multithread(
            self.n_threads, self.d_in * self.d_out, max_size=get_program().budget
        )
        def _(base, size):
        ...
        if self.print_random_update:
            ...
            print_ln(
                "%s at (%s, %s): before=%s after=%s A=%s B=%s",
                str(self.nabla_W),
                i,
                j,
                tmp[i][j].v.reveal(),
                ...
                B.get_column_by_row_indices(batch.get_vector(), i).reveal(),
            )
            print_ln(
                "batch=%s B=%s", batch, [self.X[bi][0][i].reveal() for bi in batch]
            )

        progress("nabla W")

        self.nabla_b.assign_vector(
            sum(
                sum(f_schur_Y[k][j].get_vector() for k in range(N))
        ...
        if self.debug:
            ...
            def _(i):
                ...
                def _(j):
                    ...
                    def _():
                        ...
                        print_ln(
                            "Y %s", [f_schur_Y[k][0][j].reveal() for k in range(N)]
                        )
                        print_ln("X %s", [self.X[k][0][i].reveal() for k in range(N)])
            ...
            def _(j):
                ...
                def _():
                    ...
                    print_ln("Y %s", [f_schur_Y[k][0][j].reveal() for k in range(N)])
            ...
            def _(i):
                ...
                def _(j):
                    ...
                    def _():
                        ...
                        print_ln(
                            "W %s", [self.W[k][j].reveal() for k in range(self.d_in)]
                        )
        ...
        if compute_nabla_X:
            ...
            if self.print_random_update:
                ...
                print_ln(
                    ...
                    str(self.nabla_X),
                    index,
                    self.nabla_X.to_array()[index].reveal(),
                )
    ...
    def forward(self, batch, training=False):
        if training:
            ...
            def _(i):
                ...
                self.B[i].assign_vector(
                    util.tree_reduce(
                        util.or_op,
                        (sint.get_random_bit(size=size) for i in range(n_bits)),
                    )
                )

            @for_range_opt_multithread(self.n_threads, len(batch))
            def _(i):
                self.Y[i].assign_vector(
                    1
                    / (1 - self.alpha)
                    * self.X[batch[i]].get_vector()
                    * self.B[i].get_vector()
                )
        else:
...
class ElementWiseLayer(NoVariableLayer):
    ...
    def _forward(self, batch=[0]):
        ...
        @multithread(self.n_threads, len(batch) * n_per_item, max_size=program.budget)
        def _(base, size):
    ...
    def backward(self, batch):
        ...
        @multithread(self.n_threads, n_elements)
        def _(base, size):
            self.nabla_X.assign_vector(
                self.nabla_Y.get_vector(base, size)
...
class PoolBase(NoVariableLayer):
    def __init__(
        ...
        if padding == "SAME":
            ...
            output_shape = (
                [shape[0]]
                + [
                    (shape[i + 1] + 2 * padding[i] - ksize[i + 1]) // strides[i + 1] + 1
                    for i in range(2)
                ]
                + [shape[3]]
            )
    ...
    def traverse(self, batch, process):
        ...
        @for_range_opt_multithread(self.n_threads, [len(batch), self.X.sizes[3]])
        def _(l, k):
            bi = batch[l]
            XX = self.X[bi]
                    ...
                    for ii in range(self.ksize[1]):
                        ...
                        for jj in range(self.ksize[2]):
                            ...
                            if not is_zero(h_in * w_in):
                                pool.append(
                                    [h_in * w_in * XXX[w_in * w][k], h_in, w_in, h, w]
                                )
...
class MaxPool(PoolBase):
    ...
    def forward(self, batch=None, training=False):
        ...
        def process(pool, bi, k, i, j):
            ...
            red = util.tree_reduce(m, [(x[0], [1] if training else []) for x in pool])
    ...
    def backward(self, compute_nabla_X=True, batch=None):
        if compute_nabla_X:
            ...
            def process(pool, bi, k, i, j):
                for (x, h_in, w_in, h, w), c in zip(
                    pool, self.comparisons[bi][k][i][j]
                ):
                    hh = h * h_in
                    ww = w * w_in
...
class Concat(NoVariableLayer):
    ...
    def __init__(self, inputs, dimension):
        ...
        self.Y = Tensor(shape, sfix)
        self.bases = [sum(x[dimension] for x in shapes[:k]) for k in range(len(shapes))]
        self.addresses = Array.create_from(regint(list(x.Y.address for x in inputs)))
            ...
            if len(set(self.bases)) == 1:
                ...
                def _(k):
                    self.Y[batch[0]][i][j].assign_part_vector(
                        MultiArray(self.inputs[0].shape, address=self.addresses[k])[i][
                            j
                        ].get_vector(),
                        k * self.bases[1],
                    )
...
class Add(NoVariableLayer):
    ...
    def _forward(self, batch=[0]):
        ...
        def _(base, size):
            for bb in batch:
                tmp = sum(inp.Y[bb].get_vector(base, size) for inp in self.inputs)
...
class BatchNorm(Layer):
    ...
    def __repr__(self):
        return "%s(%s, approx=%s)" % (type(self).__name__, self.X.sizes, self.approx)
    ...
    def _output(self, batch, mu, var):
        ...
        @for_range_opt_multithread(self.n_threads, [len(batch), self.X.sizes[1]])
        def _(i, j):
    ...
    def forward(self, batch, training=False):
        if training or not self.is_trained:
            ...
            def _(base, size):
                self.mu.assign_vector(s.get_vector(base, size) / (len(batch) * d), base)
            ...
            def _(base, size):
                self.var.assign_vector(
                    self.var.get_vector(base, size) / (len(batch) * d - 1), base
                )

            for (
                x,
                y,
            ) in (self.mu_hat, self.mu), (self.var_hat, self.var):
    ...
    def backward(self, batch, compute_nabla_X=True):
        factor = Array.create_from(self.InvertSqrt(self.var[:] + self.epsilon))
        ...
        def _(i, j):
            ...
            nYdf[i][j] = (
                self.nabla_Y[i][j][:] * (self.X[i][j][:] - self.mu[:]) * factor[:]
            )
        ...
        def _(base, size):
            s2.assign_vector(s2.get_vector(base, size) / (len(batch) * d - 1), base)

        @for_range_opt_multithread(self.n_threads, [len(batch), d])
        def _(i, j):
            self.nabla_X[i][j][:] = (
                mynYf[i][j][:] - s1[:] - (self.X[i][j][:] - self.mu[:]) * s2[:]
            )
...
class Conv2d(ConvBase):
    ...
    def _forward(self, batch):
        ...
        if self.use_conv2ds:
            ...
            @for_range_opt_multithread(self.n_threads, [len(batch), n_channels_out])
            def _(i, j):
                ...
                if self.bias_before_reduction:
                    ...
                    res += (
                        self.bias.expand_to_vector(j, res.size).v
                        << self.weight_squant.f
                    )
                addresses = regint.inc(
                    res.size, self.unreduced[i * part_size].address + j, n_channels_out
                )
    ...
    def backward(self, compute_nabla_X=True, batch=None):
        ...
        def _(i):
            self.nabla_bias.assign_vector(
                self.nabla_bias.get_vector()
                + sum(
                    sum(self.nabla_Y[i][j][k].get_vector() for k in range(output_w))
        ...
        batch_repeat.assign_vector(
            batch.get(regint.inc(input_size, 0, 1, 1, N))
        ...
        def _(i, j):
            a = regint.inc(
                input_size, self.X.address + i, n_channels_in, N, inputs_h * inputs_w
            )
            inputs = sfix.load_mem(batch_repeat.get_vector() + a).pre_mul()
            b = regint.inc(
                N * output_w * output_h, self.nabla_Y.address + j, n_channels_out, N
            )
            rep_out = regint.inc(output_h * output_w * N, 0, 1, 1, N) * reduce(
                operator.mul, self.output_shape[1:]
            )
        ...
        if compute_nabla_X:
            ...
            if padding_h or padding_w:
                output = MultiArray([N, padded_h, padded_w, n_channels_in], sfix)
            else:
                output = self.nabla_X

            @for_range_opt_multithread(self.n_threads, [N, n_channels_in])
            def _(i, j):
                res = sint(size=(padded_w * padded_h))
                conv2ds(
                    res,
                    self.nabla_Y[i].get_vector().pre_mul(),
        ...
        if self.debug_output:
            ...
            def _(i):
                ...
                print_ln(
                    "%s nabla Y %s %s", self, i, list(self.nabla_Y[i].reveal_nested())
                )
                if compute_nabla_X:
                    print_ln(
                        "%s nabla X %s %s",
                        self,
                        i,
                        self.nabla_X[batch[i]].reveal_nested(),
                    )

            print_ln("%s nabla weights %s", self, (self.nabla_weights.reveal_nested()))
            print_ln("%s weights %s", self, (self.weights.reveal_nested()))
...
def easyConv2d(
    input_shape, batch_size, out_channels, kernel_size, stride=1, padding=0, **kwargs
):
    ...
    output_shape = (
        [batch_size]
        + list(apply_padding(input_shape[1:3], kernel_size, stride, padding))
        + [out_channels]
    )
...
def easyMaxPool(input_shape, kernel_size, stride=None, padding=0):
    ...
    """
    kernel_size, stride, padding = _standardize_pool_options(
        kernel_size, stride, padding
    )
    return MaxPool(
        input_shape, [1] + list(stride) + [1], [1] + list(kernel_size) + [1], padding
    )
...
class FixAveragePool2d(PoolBase, FixBase):
    ...
    def __init__(
        self, input_shape, output_shape, filter_size, strides=(1, 1), padding=0
    ):
        filter_size, strides, padding = _standardize_pool_options(
...
class Optimizer:
            ...
            if i != len(self.layers) - 1 or run_last:
                ...
                if self.print_random_update:
                    ...
                    if l < 100:
                        ...
                        j = regint.get_random(64) % (layer.Y[i].total_size() - l)
                    print_ln(
                        "forward layer %s at (%s, %s): %s",
                        layer,
                        i,
                        j,
                    ...
                    print_ln(
                        "forward layer %s vertical at %s: %s",
                        layer,
                        i,
                        [layer.Y[j].to_array()[i].reveal() for j in range(len(batch))],
                    )
            if self.time_layers:
                stop_timer(100 + i)
            break_point()
    ...
    def backward(self, batch):
        ...
        for i, layer in reversed(list(enumerate(self.layers))):
            ...
            if not layer.inputs:
                layer.backward(
                    compute_nabla_X=False, batch=self.batch_for(layer, batch)
                )
            else:
                layer.nabla_X.alloc()
    ...
    def stat(cls, name, tensor):
        ...
        def _(base, size):
            ...
            tmp = Array.create_from(tensor.get_vector(base, size).reveal())
        ...
        if len(tensor.shape) == 4:
            corners = sum(([tensor[0][i][j][0] for j in (0, -1)] for i in (0, -1)), [])
                ...
                for i, layer in enumerate(self.layers[:-1]):
                    for theta in layer.thetas():
                        if theta.total_size() < 1000:
                            ...
                            self.stat(
                                " %d diff" % i,
                                Array.create_from(theta.get_vector() - old_params[0]),
                            )
                            del old_params[0]
    ...
    def run(self, batch_size=None, stop_on_loss=0):
        ...
        def _(_):
            ...
            n_per_epoch = int(math.ceil(1.0 * max(len(X) for X in self.X_by_label) / n))
            ...
            def _(j):
                ...
                if self.revealing_correctness:
                    ...
                    part_truth.assign_vector(self.layers[-1].Y.get_slice_vector(batch))
                    self.n_correct.iadd(
            ...
            if (
                self.report_loss
                and self.layers[-1].compute_loss
                and self.layers[-1].approx != 5
            ):
                print_ln(
                    "loss in epoch %s: %s",
                    i,
            ...
            if self.tol > 0:
                res *= (
                    1 - (loss_sum >= 0) * (loss_sum < self.tol * n_per_epoch)
                ).reveal()
    ...
    def reveal_correctness(self, data, truth, batch_size=128, running=False):
        ...
        def f(start, batch_size, batch):
            ...
            n_correct.iadd(self.layers[-1].reveal_correctness(batch_size, part_truth))
            loss.iadd(self.layers[-1].l * batch_size)
            if running:
                total = start + batch_size
                print_str(
                    "\rpart acc: %s (%s/%s) ",
                    cfix(n_correct, k=63, f=31) / total,
                    n_correct,
                    total,
                )
    ...
    def run_by_args(
        ...
        if "one_iter" in program.args:
            ...
            print_ln("loss")
            self.eval(
                self.layers[0].X.get_part(0, batch_size), batch_size=batch_size
            ).print_reveal_nested()
            for layer in self.layers:
        ...
        def _(i):
            if not acc_first:
                if self.time_training:
                    start_timer(1)
                self.run(
                    batch_size, stop_on_loss=0 if "no_loss" in program.args else 100
                )
                if self.time_training:
                    stop_timer(1)
            ...
            if test_X and test_Y:
                ...
                n_correct, loss = self.reveal_correctness(
                    test_X, test_Y, acc_batch_size, running="part_acc" in program.args
                )
            ...
            if acc_first:
                ...
                @if_(
                    util.or_op(
                        self.stopped_on_loss,
                        (n_correct < int(n_test // self.layers[-1].n_outputs * 1.2))
                        if test_X and test_Y
                        else 0,
                    )
                )
            ...
            print_ln_if(self.stopped_on_low_loss, "aborting run because of low loss")
    ...
    def fit(
        ...
        self.run_by_args(program, epochs, batch_size, *validation_data, reset=reset)
...
class Adam(Optimizer):
    ...
    def __init__(
        self, layers, n_epochs=1, approx=False, amsgrad=False, normalize=False
    ):
    ...
    def _update(self, i_epoch, i_batch, batch):
        ...
        for i_layer, (m, v, g, theta) in enumerate(
            zip(self.ms, self.vs, self.gs, self.thetas)
        ):
            if self.normalize:
                abs_g = g.same_shape()
                ...
                max_g = tree_reduce_multithread(
                    self.n_threads, util.max, abs_g.get_vector()
                )
                scale = MemValue(
                    sfix._new(
                        library.AppRcr(max_g.v, max_g.k, max_g.f, simplex_flag=True)
                    )
                )

            @multithread(self.n_threads, m.total_size(), max_size=get_program().budget)
            def _(base, size):
...
class SGD(Optimizer):
    ...
    def _update(self, i_epoch, i_batch, batch):
        for nabla, theta, momentum_value, delta_theta in zip(
            self.nablas, self.thetas, self.momentum_values, self.delta_thetas
        ):
            ...
            def _(base, size):
                ...
                theta.assign_vector(
                    theta.get_vector(base, size) + delta_theta.get_vector(base, size),
                    base,
                )
            ...
            if self.print_update_average:
                ...
                print_ln(
                    "update average: %s (%s)", sum(vec) * cfix(1 / len(vec)), len(vec)
                )
            if self.debug:
                ...
                def _(i):
                    x = a[i]
                    print_ln_if(
                        (x > len(batch) * limit) + (x < -len(batch) * limit),
            ...
            if self.print_random_update:
                ...
                print_ln(
                    "%s at %s: nabla=%s update=%s theta=%s",
                    str(theta),
                    index,
                    nabla.to_array().get_vector(index, l).reveal(),
...
def apply_padding(input_shape, kernel_size, strides, padding):
    ...
    if padding.lower() == "valid":
        res = (
            (input_shape[0] - kernel_size[0]) // strides[0] + 1,
        ...
        class Sequential:
            ...
            def compile_by_args(self, program):
                if "adam" in program.args:
                    ...
                    self.optimizer = "adam", [], {"amsgrad": True, "approx": False}
                else:
                    self.optimizer = "sgd", [], {}
            ...
            def build(self, input_shape, batch_size=128, program=None):
                ...
                if (
                    self.opt != None
                    and input_shape == self.opt.layers[0]._X.sizes
                    and batch_size <= self.batch_size
                    and type(self.opt).__name__.lower() == self.optimizer[0]
                ):
                    return
                ...
                for i, layer in enumerate(self.layers):
                    ...
                    if name == "dense":
                        if len(layers) == 0:
                            ...
                            N = batch_size
                            n_units = reduce(operator.mul, layers[-1].Y.sizes[1:])
                        ...
                        layers.append(Dense(N, n_units, layer[1][0], **layer[2]))
                        input_shape = layers[-1].Y.sizes
                    elif name == "conv2d":
                        input_shape = list(input_shape) + [1] * (4 - len(input_shape))
                        print(layer[1])
                        ...
                        padding = layer[1]["padding"]
                        layers.append(
                            easyMaxPool(input_shape, pool_size, strides, padding)
                        )
                        input_shape = layers[-1].Y.sizes
                    elif name == "avgpool":
                        ...
                        layers.append(
                            Dropout(
                                batch_size,
                                reduce(operator.mul, layers[-1].Y.sizes[1:]),
                                alpha=layer[1],
                            )
                        )
                        input_shape = layers[-1].Y.sizes
                ...
                if opt == "sgd":
                    ...
                    if epsilon:
                        if epsilon < opt.epsilon:
                            print(
                                "WARNING: epsilon smaller than default might "
                                "cause overflows"
                            )
                        opt.epsilon = epsilon
                elif opt == "inference":
...
def layers_from_torch(
    ...
    def process(item, inputs, input_shape, layerId, schedulerOpts, args, kwargs={}):
        if item == torch.cat:
            ...
            assert -1 in args or reduce(operator.mul, args) == reduce(
                operator.mul, input_shape
            )
            return
        ...
        if name == "Linear":
            ...
            layers.append(Dense(input_shape[0], item.in_features, item.out_features))
            if input_via is not None:
                ...
                if len(input_shape) == 4:
                    ...
                    swapped = numpy.reshape(
                        swapped, [item.out_features, input_shape[3]] + input_shape[1:3]
                    )
                    print(swapped.shape)
                ...
                layers[-1].W = sfix.input_tensor_via(input_via, swapped)
                layers[-1].b = sfix.input_tensor_via(input_via, item.bias.detach())
            ...
            input_shape = layers[-1].Y.shape
            if input_via is not None:
                shapes = [x.shape for x in (layers[-1].weights, layers[-1].bias)]
                import numpy

                swapped = numpy.moveaxis(numpy.array(item.weight.detach()), 1, -1)
                layers[-1].weights = layers[-1].weights.value_type.input_tensor_via(
                    input_via, swapped
                )
            ...
            layers.append(
                easyMaxPool(input_shape, item.kernel_size, item.stride, item.padding)
            )
            input_shape = layers[-1].Y.shape
        elif name == "AvgPool2d":
            layers.append(
                FixAveragePool2d(
                    input_shape, None, item.kernel_size, item.stride, item.padding
                )
            )
            input_shape = layers[-1].Y.shape
        elif (
            name == "AdaptiveAvgPool2d"
            or item == torch.nn.functional.adaptive_avg_pool2d
        ):
            ...
            kernel_size = [
                input_shape[1 + i] - (output[i] - 1) * stride[i] for i in (0, 1)
            ]
            layers.append(
                FixAveragePool2d(input_shape, None, kernel_size, stride, padding=0)
            )
            input_shape = layers[-1].Y.shape
            ...
            if input_via is not None:
                ...
                layers[-1].bias = sfix.input_tensor_via(input_via, item.bias.detach())
        elif name == "Dropout":
            layers.append(
                Dropout(input_shape[0], mul(layers[-1].Y.sizes[1:]), alpha=item.p)
            )
            input_shape = layers[-1].Y.sizes
        else:
    ...
    for i, layer in enumerate(torch_layers[1:-1]):
        ...
        if not layers:
            ...
            if len(layer.args) < 2 or (layer.args[1] != 1 and layer.args[1] != (1, 1)):
                args = layer.args
...
def solve_linear(
    ...
    if precond:
        return solve_linear_diag_precond(AtA, b, x, r, n_iterations, progress, stop)
...
def solve_linear_diag_precond(A, b, x, r, n_iterations, progress=False, stop=False):
    m = 1 / A.diag()
    ...
    def _(i):
        ...
        if progress:
            print_ln(
                "%s alpha=%s beta=%s r_norm=%s d_norm=%s",
                i,
                alpha.reveal(),
                beta.reveal(),
                r_norm.reveal(),
